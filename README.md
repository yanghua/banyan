...
##动态添加处理器
上面提到的 **远程配置与管控** 关键点在于：已在client的jar包中实现代码逻辑（通过client的代码逻辑读取动态配置来实现）。但zooKeeper+jvm 支持的 Class loader机制能够带来更加强大的功能：远程加载java类的字节码文件，通过这种机制，我们可以达到 **临时** 添加handler的功能。这种“打补丁”不重启的策略能够很好的对client进行临时性的透明升级！

具体的实现方式如下：
在管控端维护着client的一份源码（这个是前提条件，也是必然条件），添加新的handler，编译并生成该handler的 ***.class*** 文件，将其丢到预先配置的zookeeper节点下，然后再辅以对该handler的说明性配置。客户端感知到之后，会通过一个 `RemoteClassLoader` 将下载下来的字节码的二进制文件构建出该类的 `Class` 对象的实例，然后向上转型为 `AbstractHandler` 的实例，再加入相应的handler chain集合中，即可产生一个新的处理环节。
